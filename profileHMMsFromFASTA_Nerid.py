...
def main(args=None):
    ...
    shortOptions = "f:m:p:c:C:I:n:a:M:o:P:S:h"
    longOptions = []
    ...
    try:
        for option,optionArg in opts:
            if option=='-f':
                ...
                coverageHeuristics = (optionArg.lower() != "false") 
            elif option=='-I':
                ...
                Memory = int(optionArg)
            elif option=='-P':
				polyproteins = (optionArg.lower() != "false") 
            elif option=='-S':
                Max_sensitivity = (optionArg.lower() != "false")
            elif option=='-o':
                prefix = optionArg
            elif option=='-h':
                print __doc__
    ...
    collapsedFASTA = collapseSequence(filteredFASTA,Memory,fractionID,numCores,fractionCov=fractionCov)
    print "Performing all-by-all Diamond BLASTp of collapsed FASTA file..."
    blastResults = allByAllDiamond(collapsedFASTA,numCores,Max_sensitivity)
    if polyproteins == True:
    ...
    alignedFiles = GNU_Parl_batchMuscleCall(unalignedFiles,prefix=prefix,numCores=numCores) # Still testing.
    ...
    if not os.path.isdir("msaFiles"):
        os.mkdir("msaFiles")
    for m in alignedFiles:
        os.rename(m,"msaFiles/"+m)
    if not os.path.isdir("fastaFiles"):
        os.mkdir("fastaFiles")
    for f in allFastaFiles:	
        os.rename(f,"fastaFiles/"+f)
    if not os.path.isdir("hmmFiles"):
        os.mkdir("hmmFiles")
    for h in hmmFiles:
        os.rename(h,"hmmFiles/"+h)
    if not os.path.isdir("logFiles"):
        os.mkdir("logFiles")
    for l in filter(lambda x: (x.startswith(fastaParts[0]) or (prefix != None and x.startswith(prefix))) and x.endswith(".log"),os.listdir(os.getcwd())):
        os.rename(l,"logFiles/"+l)
    if not os.path.isdir("clusteringFiles"):
        os.mkdir("clusteringFiles")
    for c in filter(lambda x: x.startswith(fastaParts[0]) and \
                    (("allByAll" in x) or (x.endswith(".clstr")) or (x.endswith(".dmnd"))) or \
                      (x[-4:] in ('.pin','.phr','.psq')),os.listdir(os.getcwd())):
...
def collapseSequence(fastaFile,Memory,fractionID,numCores,fractionCov=None):
    """
...
def allByAllDiamond(fastaFile,numCores,Max_sensitivity=True):
    """
    ...
    BLAST on the file to itself as the Diamond-formatted database.  It
    returns an tab-delimited formatted BLAST results file name.
    """
    import os
    from subprocess import call
    formatdbCmd = "diamond makedb --in %s -d %s" % (fastaFile,(fastaFile+".dmnd"))
    print formatdbCmd
    returnVal = call(formatdbCmd.split())
    blastResults = os.path.splitext(fastaFile)[0] + "_allByAll_blastp.br"
    if Max_sensitivity == True:
        blastCmd = "diamond blastp -q %s -o %s --db %s --outfmt 6  --more-sensitive  --threads %s" % (fastaFile,blastResults,fastaFile,numCores)
    else:
        blastCmd = "diamond blastp -q %s -o %s --db %s --outfmt 6 --threads %s" % (fastaFile,blastResults,fastaFile,numCores)
...
def filterPolyproteins(blastFile):
    ...
    blackList = []
    for query,queryLength in queryInfo.items():
...
def filterSequencesOnCoverage(fastaFile,covHeurDict):
    ...
    for title,length in recordLengths.items():
...
def batchMuscleCall(fastaFiles,prefix=None):
    ...
    TBD: replace this with GNU Parallel.
    """
    from subprocess import call
    ...
    masterFh.close()
    return alignedFiles
def GNU_Parl_batchMuscleCall(fastaFiles,numCores=10,prefix=None):
    """
    This function takes in a list of FASTA files makes multiple
    sequence alignments using MUSCLE. It returns a list of the
    alignment files generated in FASTA format.
    *replace this with GNU Parallel.
    """
    from subprocess import call
    ...
    masterFh.close()
    # muscleCmd ="parallel muscle -in {} -out {.}.msa.faa -log {.}.faa.log -quiet ::: %s " % ' '.join(fastaFiles) 
    muscleCmd ="parallel -j%s muscle -in {} -out {.}.msa.faa -log {.}.faa.log -quiet ::: %s " % (numCores,' '.join(fastaFiles))
    returnVal = call(muscleCmd.split())
    print(muscleCmd)
    return alignedFiles

def batchHmmbuildCall(alignmentFiles,numCores=10,prefix=None):
    """
